...
export function Dashboard(){
  ...
  async function fetchNotes() {
    ...
    async function createNote(event) {
      ...
      if (!!data.videoURL) await Storage.put(image.name, image);
      try{
        await API.graphql({
      ...
      />
        ...
        <View>
          {filteredNotes.map((note) => (
            <><h4>{note.createdAt}</h4><SubmissionCard margin="1rem" id={note.name} description={note.description} image={note.image} /></>
          ))}
        </View>
      </View>
    )
